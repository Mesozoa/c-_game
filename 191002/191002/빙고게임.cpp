...
int main()
{
	...
	while (1)
	{
		...
		//중복체크을 위한 변수이다.기본적으로 중복되었다고 하기위해 true로 잡았다.
		bool	bAcc = true;
		...
		{
			/*
			AI Easy 모드는 현재 AI의 숫자목록중 *로 바뀌지 않은 숫자 목록을
			만들어서 그 목록중 하나를 선택하게 한다.(랜덤하게)
			*/
		case AM_EASY:
			//선택안된 숫자목록을 만들어준다.
			...
			//선택안되 목록의 숫자중 랜덤한 하나의 숫자를 얻어오기 위해
			//인덱스를 랜덤하게 구해준다.
			iInput = iNoneSelect[rand() % iNoneSelectCount];

			break;
		case AM_HARD:
			//하드 모드는 현재 숫자중 빙고줄 완성 가능성이 가장 높은 줄을
			...
			for (int i = 0; i < 5; i++)
			{
				...
				//별이 5개 미만이여야 빙고 줄이 아니고 기존에 가장많은
			...
			//모든 라인을 조사했으면 iLine에 가능서이 가장 높은 번호가 저장되었다.
