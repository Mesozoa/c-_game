...
int main()
{
	...
	while (1)
	{
		...
		//cin은 만약 오른쪽에 int변수가 오면 정수를 입력해야 한다.
		//오른쪽에 오는 변수 타입에 맞춰서 값을 입력해야 한느데 실수도
		//정수가 아닌 문자를 입력할 경우 에러가 발생한다.
		//그래서 예외처리로 에러가 발생했는지를 여기에서 체크하여
		//에러가 발생하면 cin 내부의 에러버퍼를 비워주고
		//cin 내부에 입력버퍼가 있는데 입력버퍼는 입력한 값을 저장
		//해놓고 그 값을 변수에 넣어주는 역할을 한ㄷ. 이 입력버퍼에
		//\n이 남아있으므로 버퍼를 순회하여 \n을 지워준다
		//버퍼 : 임시저장공간
		...
		if (cin.fail())
		{
			...
			//입력버퍼에 \n이 남아있으므로 입력버퍼를 검색하여 \n을 지워준다.
			//첫번째에는 검색하고 하는 버퍼 크기를 지정한다,넉넉하게 1024
