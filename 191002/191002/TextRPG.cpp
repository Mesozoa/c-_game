...
/*
1.상점에서 판매할 아이템 목록을 만들어주고 해당 목록을 출력해준다.
2.출력할때 앞에 번호를 부여해주고 번호를 입력받아서 해당 번호를 아이템을 
구입하게 해준다. 가장 마지막 번호에 뒤로가기 기능을 만든다.
단 가방이 꽉 찼거나 돈이 부족할 경우 구해하지 못하게 한다.
	...
	while (1)
	{
		...
		case MM_MAP:
			...
			while (1)
			{
				...
				//이 if문에 속한 break는 맵 메뉴를 돌려주기 위한 whiel에 속해
				//있으므로 이 while문을 빠져나간다.
				if (iMenu == MT_BACK)
					break;
				...
				while (1)
				{
					...
					case BATTLE_ATTACK:
					{
						...
						if (tMonster.iHP <= 0)
						{
							...
							cout << tMonster.iExp << " 경혐치를 획득하였습니다." << endl;
			...
			while (1)
			{
				...
				case SM_WEAPON:
					while (1)
					{
						...
						//처음에 iItemCount는 하나도 추가되어잇지 않기 때문에 0으로 초기화
						//되어 있으므로 0번 인덱스에 구매한 아이템을 추가하게 된다.그리고
						//카운트가 1이 된다.다음번에 추가할때는 1번인덱스에 추가하게된다.
		...
		case MM_INVENTORY:
			while (1)
			{
				...
				//제대로 선택했을 경우 아이템의 종류에 따라 장착 부위를
				//결정하게 된다.
				EQUIP	eq;
